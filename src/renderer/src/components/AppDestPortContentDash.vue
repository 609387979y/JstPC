<template>
  <div style="" v-loading="tabsLoading">
    ...
    <div class="destport-list" v-if="historyList.length && isHistory">
      <div class="destport-item" v-for="item in historyList" :key="item.Id" @click="selectPort(item)">
    ...
    <el-tabs class="destport-tabs" v-model="activeName" :loading="tabsLoading" @tab-click="handleClick"
      v-if="type == 1">
      <el-tab-pane v-for="item in portData" :key="item.Id" :label="item.LineName" :name="item.LineName">
        <div class="destport-list" style="padding: 0 7px 0">
          <div class="destport-item" v-for="child in item.gangkou" :key="child.Id" @click="selectPort(child)">
    ...
    <el-tabs class="destport-tabs" v-model="activeName" :loading="tabsLoading" @tab-click="handleClick"
      v-if="type == 2">
      <el-tab-pane v-for="item in portData" :key="item.LineName" :label="item.LineName" :name="item.LineName">
        <div class="destport-list" style="padding: 0 7px 0">
          <div class="destport-item" v-for="child in item.PortList" :key="child.Id" @click="selectPort(child)">
            <div class="destport-cn">{{ child.CnPortName }}</div>
            <div class="destport-en">{{ child.EnPortName }}</div>
            <div class="destport-cn">{{ child.EnChauName }}</div>
          </div>
