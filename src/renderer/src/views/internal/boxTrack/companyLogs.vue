<template>
    <JstCardLayout>
            ...
            <el-row>
                <el-col :span="8" class="page-search-form-item">
                    ...
                    <div class="page-search-form-content">
                        <el-input size="mini" v-model="state.search.UserName"> </el-input>
                    </div>
                </el-col>
                <el-col :span="8" class="page-search-form-item">
                ...
                <el-col :span="24" class="page-search-form-item">
                    ...
                    <div class="page-search-form-content">
                        ...
                        <el-button size="mini" class="search-btn" color="#409EFF" type="primary" @click="search()">查询
                        </el-button>
        ...
        <JstPageTable @change="getList" :data="data" :option="tableOption">
            ...
            <el-table-column label="用户名称" prop="RealName" min-width="120px"></el-table-column>
            <el-table-column label="购买服务" prop="TradeType" min-width="100px"></el-table-column>
            <el-table-column label="服务详情" prop="Remark" min-width="365px" show-overflow-tooltip>
                <template #default="{ row }">
                    <!-- <div class="Remark">
                        ...
                        <div>
                            <span>船公司: </span>
                            <span>{{ row.TradeParams[0].carrierCode ? row.TradeParams[0].carrierCode : '--' }}</span>
                        </div>
                        <div>
                            <span>船名: </span>
                            <span>{{ row.TradeParams[0].vessel ? row.TradeParams[0].vessel : '--' }}</span>
                        </div>
                        <div>
                            <span>航次:</span>
                            <span>{{ row.TradeParams[0].voyage ? row.TradeParams[0].voyage : '--' }}</span>
                        </div>
                        <div>
                            <span>起运港: </span>
                            <span>{{ row.TradeParams[0].polNameEn ? row.TradeParams[0].polNameEn : '--' }}</span>
                        </div>
                        <div>
                            <span>目的港: </span>
                            <span>{{ row.TradeParams[0].podNameEn ? row.TradeParams[0].podNameEn : '--' }}</span>
                        </div>
                    </div> -->
                    <div v-if="row.TradeParams.length === 1">
                        <div class="Remark" v-for="(item, index) in row.TradeParams" :key="index">
                            <div>
                                <span v-if="item.polNameEn && item.polName">
                                    {{ item.polNameEn }}({{ item.polName }}港)
                                </span>
                                <span v-else>
                                    --
                                </span>
                            ...
                            <div class="billNo">
                                <span v-if="item.containerNo">/{{ item.containerNo }}/</span>
                                <span v-else>/--/</span>
                            </div>
                            <div class="billNo">
                                <span v-if="item.billNo">{{ item.billNo }}/</span>
                                <span v-else>--/</span>
                            </div>
                            <div>
                                <span>{{ item.carrierCode ? item.carrierCode : '--' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    <div v-else>
                        <div class="Remark">
                            <div>批量导入订阅 共<span style="color:#409EFF">&nbsp;{{ row.TradeParams.length
                            }}&nbsp;</span>条
                            </div>
                        </div>
                    </div>
            ...
            <el-table-column label="数量" prop="Amount" width="120px">
                <template #default="scope">
                    <div class="count">
                        ...
                        <span :class="scope.row.Amount > 0 ? 'green' : 'red'">{{ scope.row.Amount > 0 ? '+' +
...
<script>
import { getDate } from "./components/publicUtils.js";
...
export default defineComponent({
    ...
    setup() {
        ...
        async function getList() {
            ...
            let sjc = new Date(state.search.EndTime).getTime() + 1000 * 60 * 60 * 24 - 1000;
            state.search.EndTime = sjc ? getDate(new Date(sjc), 0) : state.search.EndTime;
            console.log(search.TradeNumber);
            state.search.TradeNumber = state.search.TradeNumber.replace(/^\s+|\s+$/gm, '');
            let res = await request.get("/api/CT_TrackOcean/GetPay", {
                Page: tableOption.page,
                Limit: tableOption.pageSize,
                ...state.search
            })
            res.Data.Rows.forEach(t => {
                t.TradeParams = JSON.parse(t.TradeParams)
            })
