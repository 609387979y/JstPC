<template>
	<el-dialog title="解析舱单" v-model="jxshow" width="600px" @close="jxclose">
	...
	methods: {
	    ...
	    async uploadAnalysisHandle(file) {
	      if (file && file.file) {
	    	...
	    	var res = await this.$http.post('/api/CargoEDICommon/ParseExcel',formData);
	    	if(res.Status){
