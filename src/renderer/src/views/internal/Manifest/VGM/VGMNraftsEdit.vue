<template>
    <div class="vgm-container">
        ...
        <VGMEditor :key="editorKey" ref="vgmEditor"></VGMEditor>
    </div>
    <VGMPreviewDialog @saveDraft="saveDraft" @submit="clickSaveAndSubmit" ref="previewDialog"></VGMPreviewDialog>
    <VGMSendConfirmDialog @confirm="saveAndSubmit" ref="sendConfirmDialog"></VGMSendConfirmDialog>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        async copy() {
            ...
            let createRes = await this.$http.post("/api/CargoEDISHVGMDraft/CreateDraft",res)
            if (!createRes.Status) {
                this.$message.error(createRes.Message ?? '失败')
                return;
            }
            this.draftId = createRes.Data;
            // 重新加载数据
            await this.getData()
        ...
        async saveDraft() {
            ...
            data.Id = this.draftId
            let res = await this.$http.post("/api/CargoEDISHVGMDraft/UpdateDraft", data)
            if (!res.Status) {
                this.$message.error(res.Message ?? '失败')
                return;
            }
        ...
        async saveAndSubmit() {
            ...
            res = await this.$http.post("/api/CargoEDISHVGM/SendVGM", { draftId: this.draftId })
            if (!res.Status) {
                this.$message.error(res.Message ?? '失败')
                return;
            }
