  ...
  <el-dialog v-model="show" :before-close="beforeClose" title="发送舱单" width="521px">
    ...
    <div class="list">
      ...
      <div class="list-box">
        <div class="list-item" v-for="(item, index) in confirmData.Items">
          ...
          <div class="list-item-div">{{ item.CGO_CNNO }}</div>
          <div class="list-item-div">
            <img src="@/assets/images/icon-gold.svg" />{{ item.Amount }}
          </div>
        </div>
      </div>
      <div class="list-bottom">
        ...
        <div class="list-bottom-num">
          <img src="@/assets/images/icon-gold.svg" />{{ confirmData.TotalAmount }}
        </div>
      </div>
    ...
    <div class="btns">
      ...
      <template v-if="amountEnough">
        <el-button size="small" @click="clickConfirm" type="primary">确认发送</el-button>
      </template>
      <template v-else>
        <el-button size="small" disabled>余额不足</el-button>
        <el-button @click="toRecharge" size="small" type="primary">去充值</el-button>
      </template>
    </div>
  </el-dialog>
</template>
...
export default defineComponent({
  ...
  methods: {
    async showConfirmDialog(model) {
      let res = await this.$http.post("/api/CargoEDISHVGM/CalcVGMAmount", model);
      if (res.Status == false) {
    ...
    toRecharge() {
      this.$agent()?.ipcRequest("external/external", `/rechargeWeb?type=recharge`);
    },
  },
  activated() {
