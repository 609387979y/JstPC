...
<div class="FastCargoCreateTask" v-loading="state.dyxloading">
  <el-dialog v-model="showedit" :title="title" width="900px" top="50px" @close="cleardialog" :close-on-click-modal="false">
...
export default defineComponent({
  ...
  setup(props,context) {
	...
	function setdata(type,data){
		...
		if(state.data.payRatio){
			state.paytype = '2';
			
		}else{
			state.data.payRatio = '';
		}
		state.producttype = state.data.vesselName?'2':'3';
		state.data.includeInsurance = state.data.blQuantity?true:false;
		state.data.endRunDate = state.data.endRunDate?state.data.endRunDate.split(" ")[1]:'';
...
::v-deep .el-picker-panel__footer .el-picker-panel__link-btn.el-button--text {
  display: none;
}
</style>
<style lang="scss">
