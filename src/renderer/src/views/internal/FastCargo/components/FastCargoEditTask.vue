...
<div class="FastCargoEditTask" v-loading="state.dyxloading">
  <el-dialog v-model="showedit" width="900px" top="50px" @close="cleardialog" :close-on-click-modal="false">
   <el-form ref="form1" :model="state.data" :rules="form1rules" label-width="100px" v-if="apitype != 2">
		  ...
		  >
		    ...
		    <el-table-column label="出发地">
				<template #default="scope">
					{{scope.row.porCity?scope.row.porCity.cityFullNameCn:'-'}}/{{scope.row.porCity?scope.row.porCity.cityFullNameEn:'-'}}
				</template>
			</el-table-column>
		    <el-table-column label="目的地">
				<template #default="scope">
					{{ scope.row.fndCity?scope.row.porCity.cityFullNameCn:'-' }}/{{scope.row.fndCity?scope.row.porCity.cityFullNameEn:'-'}}
				</template>
			</el-table-column>
		...
		<el-row>
		  ...
		  <el-col :span="5">
		    <el-form-item label="件数" prop="cargoInfoQuantity">
				<el-input type="number" v-model="state.data.cargoInfoQuantity"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="5">
		    <el-form-item label="重量" prop="cargoInfoWeight">
				<el-input type="number" v-model="state.data.cargoInfoWeight"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="7">
		    <el-form-item label="体积" prop="cargoInfoVolume">
				<el-input type="number" v-model="state.data.cargoInfoVolume"></el-input>
			</el-form-item>
		  </el-col>
		  <el-col :span="17">
	...
	<div class="footer-btns">
		...
		<el-button size="mini" class="plainbtn" @click="savedrafts()" v-if="runStatus != 1">
...
export default defineComponent({
  ...
  setup(props,context) {
	...
	const showedit = ref(props.showeditd);
	...
	async function getCosApiConfig(){
		...
		porSearch(state.data.porCityName?state.data.porCityName.split(' ')[0]:'');
		fndSearch(state.data.fndCityName?state.data.fndCityName.split(' ')[0]:'');
	}
	// 关联账号选择
		...
		if(res.Data){
			...
			if(state.data.productId){
				let arr = state.productlist.filter((x)=>{
					return x.Id = state.data.productId;
				})
				nextTick(()=>{
	...
	watch(props,(old,newProps)=>{
		showedit.value = newProps.showeditd;
	})
