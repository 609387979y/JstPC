...
const getColleagueList = async () => {
    try {
        const res = await request.get("/api/CargoRate/SelectProviderRecordConfig", {
...
const getList = async () => {
    ...
    try {
        const res = await request.get('/api/CargoRate/SelectProviderRecordConfigPageRequest', {
        ...
        if (res.Status) {
            // 处理数据，提取员工信息到顶层
            const processedData = (res.Data.Rows || []).map(row => {
                const processedRow = { ...row }

                // 如果有员工信息，提取到顶层
                if (row.Employee && row.Employee.length > 0) {
                    const employee = row.Employee[0]
                    processedRow.PushPersonnel = employee.RealName || '--'
                    processedRow.MobilePhone = employee.MobilePhone || '--'
                    processedRow.ContactEmail = employee.Email || '--'
                    processedRow.DepartmentName = employee.OrgName || '--'
                } else {
                    processedRow.PushPersonnel = '--'
                    processedRow.MobilePhone = '--'
                    processedRow.ContactEmail = '--'
                    processedRow.DepartmentName = '--'
                }
...
const removeColleague = async (item) => {
    try {
        ...
        const res = await request.get("/api/CargoRate/DeleteProviderRecordConfig", {
            Id: item.Id
        })
...
const confirmAddColleague = async () => {
    ...
    try {
        const res = await request.post("/api/CargoRate/CreateProviderRecordConfig", {
            Type: false,
            CompanyId: employee.value.CompanyId,
